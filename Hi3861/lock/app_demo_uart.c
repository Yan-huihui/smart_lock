...
#define BACKGROUNDNO 0x02

UartDefConfig uartDefConfig = {0};
unsigned char uartGetFrameBuff[3] = {0};
int counter = 0;
...
static hi_void *UartDemoTask(char *param)
{
    ...
    for (;;) {
        ...
        if ((uartDefConfig.g_uartLen > 0) && (uartBuff[0] == 0xaa) && (uartBuff[1] == 0x55)) {
            if(uartBuff[5] == 0x01){
                printf("get one frame to set Face_flag\n");
            }

            /*串口读到3516每一帧分类结果放入uartGetFrameBuff*/
            if(counter < 3){
                uartGetFrameBuff[counter] = uartBuff[5];
                counter++;
            }else{
                uartGetFrameBuff[0] = uartGetFrameBuff[1];
                uartGetFrameBuff[1] = uartGetFrameBuff[2];
                uartGetFrameBuff[2] = uartBuff[5];
            }

            /*有三帧结果相同，但不是背景开锁*/
            if(counter > 2){
                if(uartGetFrameBuff[2] != BACKGROUNDNO && uartGetFrameBuff[0] == uartGetFrameBuff[1] && uartGetFrameBuff[1] == uartGetFrameBuff[2]){
                    printf("get three same frame to set Face_flag\n");
                    FACE_FLAG = 1;
                    counter = 0;
                    memset(uartGetFrameBuff, 0, sizeof(uartGetFrameBuff));
                }
            }
            printf("uartGetFrameBuff: %02x %02x %02x\n",uartGetFrameBuff[0], uartGetFrameBuff[1], uartGetFrameBuff[2]);
